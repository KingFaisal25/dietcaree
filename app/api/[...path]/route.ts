...
async function proxyRequest(req: NextRequest, backendPath: string): Promise<NextResponse> {
  ...
  // FIX KRITIS: Pastikan header Cookie dari browser di-forward ke backend.
  // Ini diperlukan agar Laravel bisa membaca laravel_session dan XSRF-TOKEN
  // untuk validasi CSRF. Tanpa ini, setiap POST/PUT/DELETE akan mendapat 419.
...
export async function GET(req: NextRequest, context: RouteContext) {
  ...
  return proxyRequest(req, `api/${path.join('/')}`);
}
